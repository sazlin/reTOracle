...
def _connect_db():
    try:
        ...
        logger.error("Error connect to DB: " + str(x.args))
...
def _execute_query(sql, args=None, need_fetch=True):
    ...
    try:
        cur = _get_cursor()
        logger.debug("SQL STRING: {}".format(sql))
        logger.debug("SQL ARGS: {}".format(args))
        ...
        if need_fetch:
            ...
            try:
                ...
                logger.error("Error dumping json for: ", results, " Args: ", x.args)
            else:
                return json_results
    except psycopg2.Error as x:
        # this will catch any errors generated by the database
        logger.info("*" * 40)
        logger.error("Error execuring query against DB: ", x.args)
